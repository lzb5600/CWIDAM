...
if __name__ == '__main__':   
    ...
    p = np.load("./best_para/scalenet_sensor_best_new.npy")
    results_mean = []
    results_std = []
    for i,missing_rate in enumerate([0.1,0.3,0.5,0.7,0.9]):
        ...
        results_mean.append(np.mean(test_acc))
        results_std.append(np.std(test_acc))
    np.save("./results/scalenet_sensor_results_new_200ep_0.1lr", np.vstack((np.array(results_mean),np.array(results_std))))
