...
def getobj(trainD, U, V, MTr, I, eta1, eta2, y0, sv):
 ...
 #   L = np.trace(multi_dot([np.dot(U.T, V), np.dot(MTr, MTr.T)/n + eta1 * I, np.dot(V.T, U)])) + eta2/n * np.trace(I1 - multi_dot([np.diag(y0[sv]), trainD.T[sv], U.T, V, MTr.T[sv].T]))
...
#    m = linalg.svd(M_c,full_matrices=1,compute_uv=0)
    sigma = s[s>0][-1]
    n = eta1_c
    C = 1/ (sigma**2 * n)
...
#    te_b = np.ones((test_data.shape[0]))*1
    trdata = np.column_stack((train_data,tr_b))
#    tedata = np.column_stack((test_data,te_b))
